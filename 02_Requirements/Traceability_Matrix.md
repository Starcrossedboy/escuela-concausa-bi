...
| REQ | Historias | Evidencia de prueba | DevLog | Estado |
|---|---|---|---|---|
| `REQ-001` · `REQ-003` | `US-104`, `US-113`, `US-313` | **BUG-026**: ningún juego de fixtures ejercita el grano escuela multi-ciclo ⬜ · verificado en local que `--desde-gold` falla con 1 solo ciclo · solape de CCT medido en Postgres: histórico 3/30 contra `dim_escuela`, actual 59/60 · descartado el arreglo aparente (repuntar `ref()` al histórico daría un modelo en verde con 3 escuelas, sin error) · consecuencia: el entrenamiento de ML-01 a grano escuela **no está cubierto por CI** | [[_DevLog/2026-08-28-marina-garcia-revision-main-bugs]] | 🔴 Bloquea `US-212` |
| `REQ-003` | `US-311`, `US-313` | **BUG-013** replanteado a `parcial`: mitad C3 ✅ (`--desde-gold`, Héctor) y mitad C1 ✅ con datos reales (4 ciclos, Diana, 149/149); lo que queda es reproducibilidad y pasa a BUG-026 · registrado además que el error de sklearn abierto en el DevLog de Diana **ya es BUG-015**, arreglado 2 h después de su corrida (verificado con `git merge-base`) | [[_DevLog/2026-08-28-marina-garcia-revision-main-bugs]] | 🟡 En progreso |
| `REQ-002` | `US-212`, `US-221` | `test_semantic_db03_db04` ✅ 29/29 sobre `main` de hoy · ningún archivo canónico de DB-03/DB-04 tocado en 123 commits · **cero colisiones de nombre** entre US-221 y DB-03/DB-04 · **BUG-027**: los 5 `sql_ref` de `metrics_kpis_base_us221.yaml` apuntan a `superset/sql/`, que ya no existe, y `test_kpis_us221` codifica la ruta a mano, así que CI no puede verlo ⬜ | [[_DevLog/2026-08-28-marina-garcia-revision-main-bugs]] | 🟡 En progreso |
| `REQ-002` · `REQ-003` | `US-212`, `US-104` | **BUG-017 / ADR-007** afectan directamente a DB-03/DB-04: con el target en alumnos absolutos y la sigmoide calibrada sobre fracción, `en_riesgo` (DEC-006, umbral 0.6) marcaría el 100 % de las escuelas y `pct_escuelas_en_riesgo` diría 100 % ⬜ · solicitada la incorporación de la dueña de los tableros a la ratificación del ADR, hoy limitada a C1/C3/C4 | [[_DevLog/2026-08-28-marina-garcia-revision-main-bugs]] | ⬜ Pendiente de ADR-007 |
