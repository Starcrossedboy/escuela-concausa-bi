...
| US-415 | in_progress | 2026-08-26 | — | Juan Carlos Macías declara la historia completa y verificada en local (`src/api/schemas_ml.py`: contrato interno entre `gold.features_escuela` y las salidas de ML-01/02/03, reutilizando el `FeaturesEscuela` canónico de `src/modelos/contrato.py`; 11 pruebas; §7 nuevo en [[03_Architecture/API_Specification]]). **No se registra `in_review`**: el PR aún no está abierto, así que no hay evidencia verificable en el repositorio. **Valida: Christian Ruiz** (TL C4), con **Karla Monter** enterada por ser dueña del contrato | 2026-08-26 |
