...
## 1. Propósito
El objetivo de **ML-01** es realizar una regresión supervisada para predecir la variación proporcional de matrícula por escuela (o por clúster municipio × nivel). Su salida no acotada es transformada posteriormente en un **índice de riesgo** acotado al intervalo `[0,1]`. Este es el primer nivel del target híbrido.

## 2. Features de Entrada
- Características históricas de matrícula por centro de trabajo (CCT).
- Características geográficas (municipio, entidad).
...
## 3. Métrica Obtenida
- **Métricas principales**: MAE (Mean Absolute Error) y RMSE (Root Mean Squared Error).
- Según la estrategia definida (`ML_Strategy.md`), se exige un desempeño mínimo en backtesting de `MAE < 0.03` y `RMSE < 0.05`.
...
- **NO usar a nivel individual (alumno)**. La granularidad de entrenamiento es la escuela o agregación por municipio × nivel.
