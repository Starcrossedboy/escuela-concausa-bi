...
## 1. Propósito
El objetivo de **ML-02** es realizar una clasificación multiclase supervisada para identificar cuál de los 6 drivers principales (D1 a D6) explica mejor el riesgo de abandono o la variación de matrícula en una escuela determinada. Es el **corazón prescriptivo del proyecto**, ya que permite emitir recomendaciones diferenciadas para dos escuelas con el mismo índice de riesgo.

## 2. Features de Entrada
- Columnas provenientes de `gold.features_escuela`.
- Indicadores demográficos, presupuestales y académicos.
...
## 3. Métrica Obtenida
- **Métricas principales**: F1 Score (Macro) y Precision por clase.
- **Umbrales requeridos**: `F1 macro >= 0.60` y `Precision >= 0.50` en cada una de las 6 clases para asegurar que no hay sesgo masivo hacia la clase mayoritaria.
- **Explicabilidad**: Integra `SHAP (KernelExplainer)` obligatorio en el pipeline de MLflow para justificar por qué se seleccionó el driver dominante.
