...
Punto de partida: [[03_Architecture/Data_Model]] §4.3 declara el **grano** de `cubo_escuela_360` y
`cubo_comparador_municipio` pero **no sus columnas**, y [[04_UX_Design/Screen_Specs]] cataloga KPI-01…KPI-14
sin ningún KPI propio de DB-03. Ese hueco es justo el alcance de US-211a.

## Qué se hizo

- **`04_UX_Design/Cube_Specs_DB03_DB04.md`** (nuevo, `DOC-CUBESPEC-DB0304`): contrato semántico completo
...
- **Trazabilidad:** fila de REQ-002 en [[02_Requirements/Traceability_Matrix]] (arquitectura, test, DevLog,
  estado 📋 → 🟡), alta en [[04_UX_Design/_index]] y §9 del plan de sprint.
...
- **Archivos creados/modificados:**
  - `04_UX_Design/Cube_Specs_DB03_DB04.md` (nuevo)
  ...
  - `04_UX_Design/_index.md`
  - `12_Roadmap_Sprints/Sprints/2-marina-garcia-del-buey.md`
  - `_DevLog/2026-08-14-marina-garcia-cubos-db03-db04.md` (nuevo) · `_DevLog/_index.md`
...
- **Frenos aplicados por el agente:** no se tocó `dbt/` (la materialización de los cubos es US-113 de C1),
  ni `03_Architecture/Data_Model.md` (dueña: Diana), ni `04_UX_Design/Screen_Specs.md` (dueño: Manuel),
  ni `_Meta/` (PM). Todo eso quedó como **solicitud formal** en §8 del contrato.
...
- **Bug reportado al PM:** `_Meta/scripts/vault_lint.py` truena con `UnicodeEncodeError` al imprimir la
  sección de huérfanos en consolas Windows (cp1252). El vault **sí** queda limpio (exit 0). Workaround:
  `$env:PYTHONIOENCODING="utf-8"`. `_Meta/**` está fuera del alcance de Marina: **no se corrigió**.
...
- **PM:** `DEC-005` (umbral 0.6) sigue sin registrarse en [[10_Risk_Governance/Decision_Log]]; el log llega
