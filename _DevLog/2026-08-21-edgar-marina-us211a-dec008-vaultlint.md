...
→ [[_DevLog/_index|Volver al índice]] · [[10_Risk_Governance/Decision_Log]] · [[03_Architecture/Data_Model]]
...
## Qué se hizo
1. **US-211a → `done`** (Execution_Status): 100%, PR de cierre aprobado, [[_DevLog/2026-08-21-marina-garcia-cierre-us211a]].
2. **`DEC-008` registrado** (Decision_Log): cambio de grano de `gold.cubo_comparador_municipio`
   ...
   Decisión de esquema de Diana (14-ago, regla 7) que [[03_Architecture/Data_Model]] §4.3 marcaba como
...
4. **`vault_lint.py` — fix Windows** (`_Meta/`): las consolas cp1252 lanzaban `UnicodeEncodeError` al imprimir
