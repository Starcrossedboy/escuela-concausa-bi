"""BUG-050 — el filtro de ciclo nace sin valor por defecto y triplica los totales.
...
para el filtro de Ciclo escolar (AC-002.2), pero `_filtros_nativos()` en
`superset/sync_semantic_layer.py` nunca leía ese campo ni fijaba un
`defaultDataMask`: el filtro nacía sin preselección y, con
`enableEmptyFilter=False` + `multiSelect=True`, Superset preseleccionaba
TODAS las opciones (los 3 ciclos), triplicando cualquier métrica `SUM()`
aguas abajo (confirmado en vivo: `total_escuelas` de DB-07 mostraba 25,578
en vez de 8,382 — el mismo patrón que BUG-044 en la API, pero a nivel de
visualización de Superset, que no pasa por la API).
...
doble que simula la respuesta real de `/api/v1/chart/data`.
"""
...
def test_default_ultimo_ciclo_resuelve_defaultdatamask(sync, monkeypatch) -> None:
    ...
    filtros = sync._filtros_nativos(
        "t", cfg, {"db07_cubo_completitud": "uuid-1"}, {"db07_cubo_completitud": 3},
    )
