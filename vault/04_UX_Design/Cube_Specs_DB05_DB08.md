...
> · Plantilla: [[vault/04_UX_Design/Cube_Specs_DB03_DB04]] (US-211a, Marina García del Buey)
...
|---|---|---|
| **Modelar** métricas, jerarquías y granos (este documento) | Monserrat Xcaret Miranda Olivas (C2) | `vault/04_UX_Design/Cube_Specs_DB05_DB08.md` |
...
| # | Regla | Origen |
|---|---|---|
| R1 | **Las salidas de ML se leen siempre por `JOIN`**, nunca como columna del hecho. `indice_riesgo` vive en `gold.predicciones` (`modelo = 'ML-01'`); `driver_dominante`, `recomendacion` y `prioridad` en `gold.recomendaciones`. Se unen por `cct, id_ciclo`. | [[vault/03_Architecture/Data_Model]] §4.1 |
| R2 | **`SIN_DATO` explícito: nunca cero, nunca nulo silencioso.** Prohibido `COALESCE(<driver>, 0)`. Toda métrica de driver viaja con su bandera de cobertura. | [[vault/03_Architecture/Data_Model]] §1 · Screen_Specs P2 |
...
| R4 | **Llaves:** `cct` (10 caracteres), `cve_mun` (5 dígitos INEGI = `cve_ent`(2) + municipio(3)), `id_ciclo`, `id_driver` (`D1`…`D6`). | [[vault/03_Architecture/Data_Model]] §9 |
...
catálogo canónico ([[vault/04_UX_Design/Screen_Specs]] §4), y la pregunta abierta del §8.3 queda
...
| **Original solicitado** | `driver × municipio × ciclo` ([[vault/03_Architecture/Data_Model]] §4.3) — **sin `nivel`**, reemplazado por DEC-009 (ver §8.1) |
...
| D3 | Infraestructura | CEMABE (DS-03) | escuela | Real | [[vault/03_Architecture/ADRs/ADR-005-dim-driver-mapeo]] |
| D4 | Conectividad | CEMABE (DS-03) | escuela | Real | [[vault/03_Architecture/ADRs/ADR-005-dim-driver-mapeo]] |
| D5 | Agua | CONAGUA SINA (DS-06) | escuela (IDW, radio 15km) | **`SIN_DATO` 100%** — sin `bronze.conagua` real todavía | [[vault/03_Architecture/ADRs/ADR-006-idw-calidad-aire-agua]] |
| D6 | Aire (PM2.5) | SINAICA (DS-05) | escuela (IDW, radio 15km) | Real | [[vault/03_Architecture/ADRs/ADR-006-idw-calidad-aire-agua]] |
...
|---|---|---|---|---|
| Lateral | **DB-05** | DB-07 | `id_driver` | ✅ Ratificada ([[vault/04_UX_Design/Screen_Specs]] §3) |
...
| **Grano en el esquema canónico hoy** | `cct × driver × ciclo` ([[vault/03_Architecture/Data_Model]] §4.3) — coincide, **sin cambio de grano** |
...
El catálogo canónico de [[vault/04_UX_Design/Screen_Specs]] asigna **KPI-07** (Driver dominante —
...
- **Consume:** [[vault/03_Architecture/Data_Model]] §4 · [[vault/04_UX_Design/Screen_Specs]] §2, §4 ·
  `dbt/seeds/dim_driver.csv` · [[vault/03_Architecture/ADRs/ADR-005-dim-driver-mapeo]] · [[vault/03_Architecture/ADRs/ADR-006-idw-calidad-aire-agua]]
