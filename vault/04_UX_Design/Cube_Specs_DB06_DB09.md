...
> · Grano dual de `gold.predicciones`: [[vault/_DevLog/2026-08-23-diana-alvarez-dec010-grano-dual-predicciones|DEC-010]]
...
|---|---|---|
| **Modelar** métricas, jerarquías y granos (este documento) | Manuel Alejandro Serranía Reinada (C2) | `vault/04_UX_Design/Cube_Specs_DB06_DB09.md` |
| **Materializar** los cubos en Gold (`dbt`) | Deni Garrido Fragoso (C1) · **US-113** | `dbt/` |
| **Esquema canónico** de Gold (`gold.predicciones` con grano dual) | Diana Aracely Alvarez Varela (C1) · **DEC-010** | [[vault/03_Architecture/Data_Model]] |
| **Publicación de salidas ML** (`gold.predicciones` / `gold.recomendaciones`) | Héctor Morales Marbán (C3) · US-313 | `src/modelos/` · `vault/15_ML_Models/Publicacion_Gold` |
...
| # | Regla | Origen |
|---|---|---|
| R1 | **Las salidas de ML se leen siempre por `JOIN`**, nunca como columna del hecho. `indice_riesgo`, `valor` (variación proyectada), `probabilidad` viven en `gold.predicciones` (`modelo = 'ML-01'`); `driver_dominante`, `recomendacion` y `prioridad` en `gold.recomendaciones`. Se unen por `cct, id_ciclo`. | [[vault/03_Architecture/Data_Model]] §4.1 |
| R2 | **`SIN_DATO` explícito: nunca cero, nunca nulo silencioso.** Prohibido `COALESCE(<driver>, 0)`. Toda métrica de ML viaja con `cobertura_prediccion` / `cobertura_recomendacion`. El **único 0 permitido** es el componente aditivo `recomendacion_emitida` (US-113). | [[vault/03_Architecture/Data_Model]] §1 · Screen_Specs P2 |
...
Las fórmulas **no se duplican**: este documento referencia el catálogo de [[vault/04_UX_Design/Screen_Specs]]
...
- **Consume:** [[vault/03_Architecture/Data_Model]] §4 · [[vault/04_UX_Design/Screen_Specs]] §4 (KPI-01…05, 07, 11, 12) ·
  [[vault/15_ML_Models/Publicacion_Gold]] · DEC-010 (grano dual) · DEC-009/DEC-008 (componentes aditivos)
