...
**Pendiente de verificación en máquina con Docker/Postgres** (Diana, antes de dar el fix
por cerrado del todo): `cargar_bronze_fixture.py --esquema coneval_irs` y
`--esquema coneval_pobreza`, luego `dbt run --select rezago_municipio` + `dbt test`, y
`pytest tests/ -q` completo.
