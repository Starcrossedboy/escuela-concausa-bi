...
| BUG-048 | **`dbt/models/sources.yml` seguía apuntando `bronze.sesnsp` al identifier del fixture (`sesnsp_test`) aunque el cargador real ya existía.** `identifier: "{{ var('bronze_sesnsp_identifier', 'sesnsp_test') }}"` — cualquier `dbt run` sin pasar la var leía el fixture (≤500 filas) en vez de `bronze.sesnsp` (real, 12,553,440 filas, 32 entidades, ya validado por Luis García 24-ago con Great Expectations 14/15). Bloqueaba de fondo la completitud real de D2 (inseguridad) en `gold.fact_escuela_ciclo`. Detectado por Diana Alvarez verificando su propia sospecha ("¿el D2 es mío?") antes de escalarlo a Luis García, dueño de DS-04 | **high** | **fixed (parcial)** | US-113 / US-122b / REQ-001 | `165c911` en `dev/diana-alvarez`: default corregido a `'sesnsp'`. Al destaparse, reveló un segundo hueco no resuelto en este fix: D2 también depende de la población de CONAPO (DS-08), que sigue en fixture (`bronze.conapo_sample`, 36 filas) — bloqueado en Emilio Galnares, ver detalle | verificado real contra Postgres: `dbt run` sin `--vars` → `silver.delitos_municipio` cruza 2,486 municipios; `comp_media` de D2 estable (0.4861/0.4827/0.4804), sin regresión. Sin prueba automatizada nueva — fix de default de config, verificación manual (ver DevLog 2026-09-05) |
