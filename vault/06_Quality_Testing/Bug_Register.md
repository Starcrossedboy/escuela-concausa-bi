...
`tests/test_frontend_dashboards_streamlit.py` — **33 passed**, incluye 3 que el CI se
saltaba (por no instalar streamlit) y el fix real de la causa raíz del test en secuencia:
...
(33). En la suite observable completa del ambiente se ven además 21 `failed` en tests de
validación (`great_expectations` — `'project_root_dir' and 'context_root_dir' are conflicting
args`, conflicto de versión de la librería, preexistente y ajeno a este PR) y 12 módulos que
no colectan por faltar el módulo `limits` (slowapi de `src/api`). `vault_lint` limpio.
