...
| BUG-049 | La **etiqueta del tab activo de DB-05** (`D1 · Pobreza y rezago social`) queda en **4.07:1** de contraste sobre su fondo en tema oscuro, por debajo del mínimo AA de 4.5:1 para texto normal (14 px, no es texto grande). Medido sobre el color y el fondo **efectivos** del nodo, no los declarados. Es el color de acento del tab seleccionado de Superset, no un color propio del tablero — pero el texto lo introdujo US-213, y es el control con el que se navegan los 6 drivers, así que se registra en vez de descartarlo como "de un tercero". En el mismo barrido, 30 de 32 nodos de texto **sí** cumplen AA; el otro que falla es `Edit dashboard` (3.41:1), chrome de Superset explícitamente fuera del alcance del plan | low | open | US-215b / US-213 / REQ-002 | pendiente — no se corrige por cuenta propia: tocar el acento de los tabs afecta a **todos** los tableros, no sólo DB-05, así que es decisión de la convención de la capa semántica (Manuel Serranía, US-202) y del sistema de diseño. Nota estructural: `vault/04_UX_Design/UX_Guidelines.md` está **vacío** con `source_of_truth: true`, así que hoy no existe una paleta oficial contra la cual ratificar un color de reemplazo | Reportado por Monserrat Miranda (2026-09-04) verificando §3.1 de US-215b en navegador real, tras el fix de BUG-038. **Tema claro sin medir todavía.** Sin test de regresión: el proyecto no tiene CI de accesibilidad (hueco ya documentado en el propio plan de pruebas) |
