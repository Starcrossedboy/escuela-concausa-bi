...
| G9 Plantilla de PR | verificar_plantilla_pr.sh | cada PR | ✅ | ✅ Implementado |
| G10 Sincronía con `main` | `git merge-base --is-ancestor` | cada PR | ✅ | ✅ Implementado |
| G11 Identidad, rama y alcance | check_ownership.py | cada PR | ✅ | ✅ Implementado |
...
jobs:
  quality:
    ...
    steps:
      ...
      - name: G7 Vault lint
        run: python vault/_Meta/scripts/vault_lint.py .
      - name: G8 Validar tablero PM
        run: python vault/_Meta/scripts/validate_pm_dashboard.py .
      - name: G1 Ruff lint
...
> **Estado actual:** 10/11 gates implementados (G3 pendiente para S3)
