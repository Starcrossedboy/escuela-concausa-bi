...
status: approved
traces_up: ["vault/12_Roadmap_Sprints/Sprints/3-carlos-guillermo-mayorga-tapia"]
...
> → [[vault/09_AI_Governance/AI_Agent_Governance]] · Plan: [[vault/12_Roadmap_Sprints/Sprints/3-carlos-guillermo-mayorga-tapia]]
...
| **Identidad** | `carlos-mayorga` |
| **Rama fija** | `dev/carlos-mayorga` — permanente, no se borra al mergear |
...
| US-324 | S5 | Una ficha por modelo en `vault/15_ML_Models/`: proposito, features de entrada, metrica obtenida, limitaciones conocidas y en que contextos NO debe usarse la prediccion. |

---

## 6. Reglas de uso de IA que aplican

- **DevLog obligatorio por sesión con IA**, antes del push (`vault/_DevLog/YYYY-MM-DD-carlos-mayorga-*.md`).
...
- **Nunca commit directo a `main`**: todo entra por PR desde su rama fija `dev/carlos-mayorga`.
...
- **Nunca `rebase` ni `--force`** sobre `dev/carlos-mayorga`.
