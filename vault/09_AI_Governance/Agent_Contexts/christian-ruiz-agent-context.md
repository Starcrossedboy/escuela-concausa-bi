...
status: approved
traces_up: ["vault/12_Roadmap_Sprints/Sprints/4-christian-imanol-ruiz-hurtado"]
...
> → [[vault/09_AI_Governance/AI_Agent_Governance]] · Plan: [[vault/12_Roadmap_Sprints/Sprints/4-christian-imanol-ruiz-hurtado]]
...
| **Identidad** | `christian-ruiz` |
| **Rama fija** | `dev/christian-ruiz` — permanente, no se borra al mergear |
...
- `vault/07_Security/**` (secretos y políticas)
...
| `requirements/celula-4.txt` | dueño del área | cambio acotado; avisar en el PR |
...
| ID | Sprint | Objetivo |
|---|---|---|
| US-401 | S1 | Especificacion de TODOS los endpoints ANTES de construir, para que las Celulas 2 y 3 trabajen en paralelo con mocks. Va a `vault/03_Architecture/API_Specification.md`. |
...
- **DevLog obligatorio por sesión con IA**, antes del push (`vault/_DevLog/YYYY-MM-DD-christian-ruiz-*.md`).
...
- **Nunca commit directo a `main`**: todo entra por PR desde su rama fija `dev/christian-ruiz`.
...
- **Nunca `rebase` ni `--force`** sobre `dev/christian-ruiz`.
