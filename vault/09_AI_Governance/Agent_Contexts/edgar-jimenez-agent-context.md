...
> → [[vault/09_AI_Governance/AI_Agent_Governance]] · Plan: [[vault/12_Roadmap_Sprints/Sprints/5-edgar-ulises-jimenez-lopez]]
...
| **Identidad** | `edgar-jimenez` |
| **Rama fija** | `dev/edgar-jimenez` — permanente, no se borra al mergear |
...
- **DevLog obligatorio por sesión con IA**, antes del push (`vault/_DevLog/YYYY-MM-DD-edgar-jimenez-*.md`).
...
- **Nunca commit directo a `main`**: todo entra por PR desde su rama fija `dev/edgar-jimenez`.
...
- **Nunca `rebase` ni `--force`** sobre `dev/edgar-jimenez`.
