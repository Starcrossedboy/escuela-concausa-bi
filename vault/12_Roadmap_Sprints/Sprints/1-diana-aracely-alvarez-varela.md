...
| **Formato de entrega** | Rama fija `dev/diana-alvarez` (sincronizada con `main`) → PR con plantilla y título estándar → 1 aprobación (PM) → merge a `main` |
...
| **Objetivo** | Contrato de las 3 capas: Bronze (raw + metadatos), Silver (tipado, CCT homologado) y Gold (esquema estrella). Incluye el parametro `SCOPE_ENTIDADES` y la politica de cobertura parcial. Documentar en `vault/03_Architecture/Data_Model.md`. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama fija `dev/diana-alvarez` (sincronizada con `main`) → PR con plantilla → 1 aprobación del PM → merge a `main` |
...
Trabajas siempre en **`dev/diana-alvarez`**. No creas una rama por historia, ni por sprint, ni por tema: esa
...
# 4. Sube
git push origin dev/diana-alvarez

# 5. Abre el PR en GitHub con la plantilla (se carga sola) y este título:
#    [Diana Alvarez] - Descripción concisa (US-###) - [sync|CI|DoF|DevLog]
...
`YYYY-MM-DD-diana-alvarez-descripcion.md`. Debe registrar: qué pediste, qué generó la IA, qué revisaste
...
- Tu alcance vive en dos lugares que dicen lo mismo: `vault/_Meta/ownership.yml` (lo que lee el CI) y
  tu `vault/09_AI_Governance/Agent_Contexts/diana-alvarez-agent-context.md` (la versión legible).
...
| `US-102` | Construir el DAG maestro de orquestacion e | ✅ Terminado | 100% | — | Dom 16 ago |
