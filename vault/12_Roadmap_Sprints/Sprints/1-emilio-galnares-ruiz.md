...
| **Formato de entrega** | Rama fija `dev/emilio-galnares` (sincronizada con `main`) → PR con plantilla y título estándar → 1 aprobación (PM) → merge a `main` |
...
| **Objetivo** | SOLO tus fuentes DS-06 y DS-08: descargar fisicamente, abrir, contar registros, verificar esquema y llave (clave INEGI 5 digitos) y llenar la prueba en `vault/14_Data_Sources/DS-06_CONAGUA_SINA.md` y `DS-08_CONAPO_Proyecciones.md`. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama fija `dev/emilio-galnares` (sincronizada con `main`) → PR con plantilla → 1 aprobación del PM → merge a `main` |
...
Trabajas siempre en **`dev/emilio-galnares`**. No creas una rama por historia, ni por sprint, ni por tema: esa
...
# 4. Sube
git push origin dev/emilio-galnares

# 5. Abre el PR en GitHub con la plantilla (se carga sola) y este título:
#    [Emilio Galnares] - Descripción concisa (US-###) - [sync|CI|DoF|DevLog]
...
`YYYY-MM-DD-emilio-galnares-descripcion.md`. Debe registrar: qué pediste, qué generó la IA, qué revisaste
...
- Tu alcance vive en dos lugares que dicen lo mismo: `vault/_Meta/ownership.yml` (lo que lee el CI) y
  tu `vault/09_AI_Governance/Agent_Contexts/emilio-galnares-agent-context.md` (la versión legible).
