...
| **Formato de entrega** | Rama fija `dev/manuel-serrania` (sincronizada con `main`) → PR con plantilla y título estándar → 1 aprobación (PM) → merge a `main` |
...
| **Cómo se entrega** | Rama fija `dev/manuel-serrania` (sincronizada con `main`) → PR con plantilla → 1 aprobación del PM → merge a `main` |
...
| **Objetivo** | Arquitectura de informacion de los 10 tableros, arbol de navegacion y definicion formal de cada KPI con su formula SQL. Documentar en `vault/04_UX_Design/Screen_Specs.md`. |
...
Trabajas siempre en **`dev/manuel-serrania`**. No creas una rama por historia, ni por sprint, ni por tema: esa
...
# 4. Sube
git push origin dev/manuel-serrania

# 5. Abre el PR en GitHub con la plantilla (se carga sola) y este título:
#    [Manuel Serrania] - Descripción concisa (US-###) - [sync|CI|DoF|DevLog]
...
`YYYY-MM-DD-manuel-serrania-descripcion.md`. Debe registrar: qué pediste, qué generó la IA, qué revisaste
...
- Tu alcance vive en dos lugares que dicen lo mismo: `vault/_Meta/ownership.yml` (lo que lee el CI) y
  tu `vault/09_AI_Governance/Agent_Contexts/manuel-serrania-agent-context.md` (la versión legible).
