...
| **Formato de entrega** | Rama fija `dev/andres-gonzalez` (sincronizada con `main`) → PR con plantilla y título estándar → 1 aprobación (PM) → merge a `main` |
...
| **Cómo se entrega** | Rama fija `dev/andres-gonzalez` (sincronizada con `main`) → PR con plantilla → 1 aprobación del PM → merge a `main` |
...
Trabajas siempre en **`dev/andres-gonzalez`**. No creas una rama por historia, ni por sprint, ni por tema: esa
...
# 4. Sube
git push origin dev/andres-gonzalez

# 5. Abre el PR en GitHub con la plantilla (se carga sola) y este título:
#    [Andres Gonzalez] - Descripción concisa (US-###) - [sync|CI|DoF|DevLog]
...
`YYYY-MM-DD-andres-gonzalez-descripcion.md`. Debe registrar: qué pediste, qué generó la IA, qué revisaste
...
- Tu alcance vive en dos lugares que dicen lo mismo: `vault/_Meta/ownership.yml` (lo que lee el CI) y
  tu `vault/09_AI_Governance/Agent_Contexts/andres-gonzalez-agent-context.md` (la versión legible).
...
| `US-303` | Registrar los 3 modelos en MLflow y exponerlos vía API | 🟡 En curso | 90% | Registry compartido de C5 · integración final de inferencia C4 | Dom 30 ago |
