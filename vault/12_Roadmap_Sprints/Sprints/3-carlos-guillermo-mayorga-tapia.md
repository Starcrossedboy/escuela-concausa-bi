...
| **Formato de entrega** | Rama fija `dev/carlos-mayorga` (sincronizada con `main`) → PR con plantilla y título estándar → 1 aprobación (PM) → merge a `main` |
...
| **Cómo se entrega** | Rama fija `dev/carlos-mayorga` (sincronizada con `main`) → PR con plantilla → 1 aprobación del PM → merge a `main` |
...
|---|---|
| **Sprint** | S5 — Lun 31 ago - Dom 6 sep |
| **Objetivo** | Una ficha por modelo en `vault/15_ML_Models/`: proposito, features de entrada, metrica obtenida, limitaciones conocidas y en que contextos NO debe usarse la prediccion. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama fija `dev/carlos-mayorga` (sincronizada con `main`) → PR con plantilla → 1 aprobación del PM → merge a `main` |
...
Trabajas siempre en **`dev/carlos-mayorga`**. No creas una rama por historia, ni por sprint, ni por tema: esa
...
# 4. Sube
git push origin dev/carlos-mayorga

# 5. Abre el PR en GitHub con la plantilla (se carga sola) y este título:
#    [Carlos Mayorga] - Descripción concisa (US-###) - [sync|CI|DoF|DevLog]
...
`YYYY-MM-DD-carlos-mayorga-descripcion.md`. Debe registrar: qué pediste, qué generó la IA, qué revisaste
...
- Tu alcance vive en dos lugares que dicen lo mismo: `vault/_Meta/ownership.yml` (lo que lee el CI) y
  tu `vault/09_AI_Governance/Agent_Contexts/carlos-mayorga-agent-context.md` (la versión legible).
