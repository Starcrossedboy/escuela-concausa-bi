...
| **Formato de entrega** | Rama fija `dev/hector-morales` (sincronizada con `main`) → PR con plantilla y título estándar → 1 aprobación (PM) → merge a `main` |
...
| **Cómo se entrega** | Rama fija `dev/hector-morales` (sincronizada con `main`) → PR con plantilla → 1 aprobación del PM → merge a `main` |
...
| **Objetivo** | Tabla comparativa, curvas y analisis de error por entidad. Va a `vault/06_Quality_Testing/`. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama fija `dev/hector-morales` (sincronizada con `main`) → PR con plantilla → 1 aprobación del PM → merge a `main` |
...
Trabajas siempre en **`dev/hector-morales`**. No creas una rama por historia, ni por sprint, ni por tema: esa
...
# 4. Sube
git push origin dev/hector-morales

# 5. Abre el PR en GitHub con la plantilla (se carga sola) y este título:
#    [Hector Morales] - Descripción concisa (US-###) - [sync|CI|DoF|DevLog]
...
`YYYY-MM-DD-hector-morales-descripcion.md`. Debe registrar: qué pediste, qué generó la IA, qué revisaste
...
- Tu alcance vive en dos lugares que dicen lo mismo: `vault/_Meta/ownership.yml` (lo que lee el CI) y
  tu `vault/09_AI_Governance/Agent_Contexts/hector-morales-agent-context.md` (la versión legible).
