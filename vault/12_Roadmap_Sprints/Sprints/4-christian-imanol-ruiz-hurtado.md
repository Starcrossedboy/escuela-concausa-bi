...
| **Quién revisa tu código** | Edgar Edmundo Coronel Navarrete (PM) — compuerta única (DEC-003). Christian Imanol Ruiz Hurtado (Tech Lead) revisa como apoyo, no bloquea |
| **Formato de entrega** | Rama fija `dev/christian-ruiz` (sincronizada con `main`) → PR con plantilla y título estándar → 1 aprobación (PM) → merge a `main` |
...
| **Cómo se entrega** | Rama fija `dev/christian-ruiz` (sincronizada con `main`) → PR con plantilla → 1 aprobación del PM → merge a `main` |
...
| **Objetivo** | Especificacion de TODOS los endpoints ANTES de construir, para que las Celulas 2 y 3 trabajen en paralelo con mocks. Va a `vault/03_Architecture/API_Specification.md`. |
...
Trabajas siempre en **`dev/christian-ruiz`**. No creas una rama por historia, ni por sprint, ni por tema: esa
...
# 4. Sube
git push origin dev/christian-ruiz

# 5. Abre el PR en GitHub con la plantilla (se carga sola) y este título:
#    [Christian Ruiz] - Descripción concisa (US-###) - [sync|CI|DoF|DevLog]
...
- Solicita a **Christian Imanol Ruiz Hurtado** (Tech Lead) como revisor de apoyo si el cambio toca su área; su revisión no bloquea el merge.
...
`YYYY-MM-DD-christian-ruiz-descripcion.md`. Debe registrar: qué pediste, qué generó la IA, qué revisaste
...
- Tu alcance vive en dos lugares que dicen lo mismo: `vault/_Meta/ownership.yml` (lo que lee el CI) y
  tu `vault/09_AI_Governance/Agent_Contexts/christian-ruiz-agent-context.md` (la versión legible).
