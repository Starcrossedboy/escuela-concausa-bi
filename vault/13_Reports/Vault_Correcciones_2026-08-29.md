...
> → [[vault/_Meta/Vault_Rules]] · [[vault/06_Quality_Testing/Bug_Register]] · [[vault/12_Roadmap_Sprints/Execution_Status]]
...
La guarda anti-mojibake de [[vault/06_Quality_Testing/Bug_Register#BUG-014]] busca secuencias
doble-codificadas (`Ã³`). No detecta el caso inverso: un byte latin-1 **que no es UTF-8 válido**.

Caso real, PR #102, en `vault/11_Operations/_index.md`:

```
| [[vault/11_Operations/Alertas_Monitoreo_US524a]] | Pol\xedtica de alertas ...
```
...
**Acción:** agregar a `vault/_Meta/scripts/vault_lint.py` una validación de decodificación UTF-8 estricta
...
ya lo ocupa [[vault/03_Architecture/ADRs/ADR-007-unidad-target-variacion-matricula]] — que además está en
...
1. Renombrar a **ADR-008** en los tres lugares: nombre de archivo, campo `id:` del frontmatter y
   título del documento. Actualizar `vault/03_Architecture/ADRs/_index.md`.
...
## V-04 · Higiene de `vault/_DevLog/_index.md` y del registro de bugs
...
`.gitattributes` declara `vault/_DevLog/_index.md merge=union` precisamente porque cada PR le agrega una
...
**Acción:** documentar el procedimiento en `vault/_Meta/Vault_Rules.md` — *ante conflicto en
`vault/_DevLog/_index.md`, resolver con `git merge origin/main` en local, nunca con el editor web*.
...
| V-04a | Fila duplicada en `vault/_DevLog/_index.md` | baja | Edgar Coronel | 02-sep |
...
[[vault/03_Architecture/ADRs/ADR-007-unidad-target-variacion-matricula]] —que destraba US-212, DB-04 y
BUG-017— y [[vault/06_Quality_Testing/Bug_Register#BUG-020]], único riesgo vivo para la casilla 6 del
ensayo E2E.
