...
## Qué se hizo
- Diagnosticado y corregido bug en `vault/_Meta/scripts/vault_lint.py`: los filtros de exclusión de directorios usaban separador Unix (`/`) que no funciona en Windows (`\`), causando 3 falsos positivos bloqueantes en el linter.
...
- **Archivos creados/modificados:** `vault/_Meta/scripts/vault_lint.py`
...
- [x] Tests agregados/actualizados (verificado con `python vault/_Meta/scripts/vault_lint.py .` → ✅ Vault limpio)
