...
traces_up: ["vault/02_Requirements/User_Stories", "vault/04_UX_Design/Screen_Specs", "vault/04_UX_Design/Cube_Specs_DB03_DB04"]
...
2. **Doc vault `vault/04_UX_Design/Superset_Setup_US202.md`** (DOC-SUPERSET-SETUP) — Cómo levantar ambiente, correr sync, verificar conexión, limitaciones conocidas (Gold no existe), workaround de psycopg2, Gold mock tables.
...
5. **Actualizaciones vault:**
   - `vault/04_UX_Design/_index.md` — Fila DOC-SUPERSET-SETUP.
   - `vault/02_Requirements/Traceability_Matrix.md` — REQ-002: referencia a Superset Setup y DevLog US-202.
   - `vault/12_Roadmap_Sprints/Sprints/2-manuel-alejandro-serrania-reinada.md` — US-202: 🟡 → 🔵 En revisión (100%).

### BUG-003 — sklearn faltante

Registrado en `vault/06_Quality_Testing/Bug_Register.md`: `test_entrenar_ml01.py` y `test_entrenar_ml02.py` fallan con `ModuleNotFoundError: No module named 'sklearn'`. Severidad low, estado open, fuera de alcance C2 (dueño: C3 Andrés).

### BUG-004 — psycopg2 no incluido en imagen Superset

Registrado en `vault/06_Quality_Testing/Bug_Register.md`: la imagen oficial `apache/superset:latest` no trae `psycopg2`. Conexión a PostgreSQL falla con 422. Severidad medium, estado open, dueño C3 (Edward Ruiz — US-522c). Workaround documentado: `pip install --target /app/.venv/lib/python3.10/site-packages psycopg2-binary`.
