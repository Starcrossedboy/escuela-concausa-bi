...
→ [[vault/_DevLog/_index|Volver al índice]] · [[vault/03_Architecture/ADRs/ADR-004-autenticacion-oauth2-jwt|ADR-004]] · [[vault/03_Architecture/API_Specification|API_Spec §2]]
...
- **Archivos creados:** `src/api/config.py`, `src/api/security/{__init__,jwt,google,roles,deps}.py`,
  `vault/03_Architecture/ADRs/ADR-004-autenticacion-oauth2-jwt.md`, `tests/test_auth_jwt.py`.
- **Modificados:** `src/api/v1/auth.py`, `src/api/app.py` (lifespan + guarda prod),
  ...
  `vault/03_Architecture/ADRs/_index.md`.
...
- **Edgar (owner de `vault/07_Security/Security_Model.md`):** actualizar la sección Autenticación (hoy es
