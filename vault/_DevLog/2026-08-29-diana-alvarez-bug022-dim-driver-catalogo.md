...
2026-08-28 ([[vault/_DevLog/2026-08-28-diana-alvarez-formato911-real-validacion-us113]]): `dbt seed
...
documentó el catálogo canónico en `vault/03_Architecture/Data_Model.md` §4.2, y se registró `BUG-022` en
...
4. Regresión más amplia, mismo rigor que BUG-021:
   ...
   pytest tests/ -q                                     # 497 passed, 1 failed (flake preexistente
                                                          ...
                                                          # 5 skipped
   python vault/_Meta/scripts/vault_lint.py .                  # Vault limpio (3 huérfanos preexistentes,
                                                          # no relacionados)
   ```
...
- `BUG-022`: causa raíz identificada y corregida con un test que falla en CI/local en vez de
  esperar a un HTTP 500 en Superset. Entrada agregada a `vault/06_Quality_Testing/Bug_Register.md` sobre
...
- Fila en `vault/02_Requirements/Traceability_Matrix.md`: pendiente (fuera del alcance de este fix
  puntual).
