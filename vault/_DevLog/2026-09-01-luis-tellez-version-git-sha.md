...
Este trabajo se preparó **apilado sobre el item 5** (PR #178) para no chocar en `vault/_DevLog/_index.md`.
Al mergearse #178 (2026-09-01) se borró su rama base y GitHub **cerró el PR apilado #179**; el item 6 se
**rebasó sobre `main`** (que ya incluye el item 5) y se reabrió como PR directo a `main`.
...
- **`vault/08_CICD_DevOps/scripts/build-and-push.sh`**: captura `GIT_SHA=$(git rev-parse HEAD)` y lo pasa como
...
- **No** se promovió nada a producción (verificado en local). Solo territorio mío: `src/api` (contrato),
  `docker/**`, `docker-compose.yml`, `.env.example`, `vault/08_CICD_DevOps/scripts/`, `tests/test_api_*`.
