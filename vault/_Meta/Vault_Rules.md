...
3. **Todo artefacto tiene un ID único** según [[vault/_Meta/Naming_Conventions]]. Los IDs nunca se reciclan.

4. **Nada vive en la raíz de una carpeta sin estar en su `_index.md` (MOC).** Ver
   [[vault/_Meta/Definition_of_Filed]].

5. **Cambios al código pasan por PR**, nunca push directo a la rama protegida. Ver
   [[vault/05_Engineering/Engineering_Workflow]].

6. **Toda sesión con IA genera una entrada de DevLog** antes del push. Ver [[vault/_DevLog/_index]].

7. **Cambios de seguridad, schema o CI/CD requieren revisión humana explícita** del dueño del área.

8. **Una rama fija por persona: `dev/{identidad}`.** Permanente, nunca se borra, nunca se recrea.
   Se sincroniza con `git merge origin/main` antes de cada cambio y otra vez antes de abrir el PR.
   Ni `rebase` ni `--force`. Ver [[vault/05_Engineering/Branching_Strategy]].

9. **Cada quien toca solo su alcance.** El padrón de identidades y permisos vive en un archivo,
   `vault/_Meta/ownership.yml`, y el CI lo hace cumplir. Para cambiar algo ajeno, se le pide a su
   dueño. Ver [[vault/09_AI_Governance/AI_Agent_Governance]].
...
- Correr `vault/_Meta/scripts/vault_lint.py`.
...
aparición —el PR #102, con 227 líneas de `vault/_DevLog/_index.md` reescritas— motivó esta regla.
...
`vault/12_Roadmap_Sprints/Execution_Status.md` apareció una vez con **112 líneas modificadas** sin un solo
